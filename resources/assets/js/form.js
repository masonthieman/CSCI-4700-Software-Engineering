...
var dynamicFormAdd = function (group, attributes = {}, hash = null) {	
    ...
    template.find("[name]").each(function () {
		...
		if(group != 'tcm_patient_contact_addl_attempt'){
			let id = $(this).attr("id");
			if(id){
				$(this).attr("id", id+'_'+hash);
				$(this).parent().find('label').attr('for', id+'_'+hash);
			}
		}
		
		if (attributes[name]==0) { // Check if attributes are supplied for this field	
	...
	if(group == 'tcm_patient_contact_addl_attempt'){
		...
		template.find('#nextAttemptstarts .custom-control-label').attr('id', 'addl_contact_attempt_label_'+counter);
		template.find('#addl_contact_attempt_label_'+counter).attr('for', 'addl_contact_attempt'+counter);
		template.find('#nextAttemptstarts #addl_contact_attempt').attr('id', 'addl_contact_attempt'+counter);
		
		template.find('#addl_contact_attempt_successful').prev('label').attr('for', 'addl_contact_attempt_successful_'+counter);
		template.find('#addl_contact_attempt_successful').attr('id', 'addl_contact_attempt_successful_'+counter);
...
var dynamicFormPopulate = function(formName, data) {
    ...
    for (var group in data.dynamic) {
        for (var hash in data.dynamic[group]) {
			//alert(JSON.stringify(data.dynamic[group][hash]));
